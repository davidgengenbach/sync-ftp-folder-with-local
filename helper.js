...
var writeJSONFileSync =  R.curry(function(filename, data) {
        fs.writeFileSync(filename, JSON.stringify(data, null, '\t'), encoding);
    });
var readJSONFileSync = R.pipe(R.curry(fs.readFileSync)(R.__, encoding), JSON.parse);

module.exports = {
    setConfigFromFile: function(configPath) {
    ...
    setCache: R.curry(function(name, value) {
        var cachePath = path.join(ftpConfig.localCache, name);
        writeJSONFileSync(cachePath, value);
    }),
    getCache: function(name) {
        return readJSONFileSync(path.join(ftpConfig.localCache, name));
    },
    getConfig: function() {
        return ftpConfig;
    },
    ...
    setupFTP: function() {
        return getFTPClientPromised(ftpConfig);
    },
...
function getFTPClientPromised(settings) {
    return new Bluebird(function(res, rej) {
        getFTPClient(settings, res);
    });
}
...
function getFiles(mode) {
    var walkPath = mode === REMOTE ? ftpsync.settings.remote : ftpsync.settings.local,
    ...
    return new Bluebird(function(res, rej) {
        ftpsync.setup(function() {
            walkFn(walkPath, function(err, files) {
                if (err) rej(err);
                else res(files);
            });
        });
